...
const UnderlineIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-underline"><path d="M6 4v6a6 6 0 0 0 12 0V4"/><line x1="4" x2="20" y1="20" y2="20"/></svg>
);
const CodeIcon = () => (
...
const MarkdownIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-markdown"><path d="M9 12v-3h6v3M9 17h6M9 12H3L6 9l-3-3h6v6zM21 12h-6l-3 3 3 3h6v-6z"/></svg>
);
...
const Heading2Icon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-heading-2"><path d="M4 12h8"/><path d="M4 18V6"/><path d="M12 18V6"/><path d="M21 18h-4c0-4 4-3 4-6 0-1.5-2-2.5-4-1"/></svg>
);
const QuoteIcon = () => (
...
export default function App() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (markdownHelpRef.current && !markdownHelpRef.current.contains(event.target as Node)) {
  ...
  const handleFormat = (formatType: 'bold' | 'italic' | 'code' | 'link' | 'ul' | 'ol' | 'underline' | 'h1' | 'h2' | 'quote' | 'hr') => {
    ...
    if (['ul', 'ol', 'h1', 'h2', 'quote'].includes(formatType)) {
        ...
        const formattedLines = lines.map(line => `${startChar}${line}`).join('\n');
        ...
        newText = `${body.substring(0, start)}${startChar}${selectedText}${endChar}${body.substring(end)}`;
    }
    
    setBody(newText);

    setTimeout(() => {
        if(selectedText.length === 0 && !['ul', 'ol', 'hr'].includes(formatType)) {
            textarea.focus();
  ...
  const handlePublish = () => {
    if (!title.trim() || !body.trim()) {
        setNotification({ type: 'error', message: 'Title and Body cannot be empty.' });
  ...
  const handleNewTagKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && newTag.trim() !== '') {
        ...
        if (!availableTags.includes(newTagTrimmed) && !selectedTags.includes(newTagTrimmed)) {
            setAvailableTags([...availableTags, newTagTrimmed]);
            setSelectedTags([...selectedTags, newTagTrimmed]);
        } else if (availableTags.includes(newTagTrimmed) && !selectedTags.includes(newTagTrimmed)) {
            setSelectedTags([...selectedTags, newTagTrimmed]);
        }
        setNewTag('');
    }
  ...
  const renderFeatherFields = () => {
    switch (feather) {
      case 'Photo':
        return (
          <div className="mt-4">
            <label className="block text-sm font-medium text-gray-400 mb-2">Upload Photo</label>
            {photoPreview ? (
                <div className="relative">
                    <img src={photoPreview} alt="Preview" className="w-full h-auto rounded-lg" />
                    <button 
                        onClick={removePhoto} 
        ...
        return (
          <div className="mt-4">
            ...
            {videoPreview ? (
                ...
                <div className="flex items-center justify-center w-full">
                  <label htmlFor="video-dropzone-file" className="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-800 hover:bg-gray-700 transition-colors">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      <VideoIcon />
                    ...
                    <input id="video-dropzone-file" type="file" className="hidden" accept="video/*" onChange={handleVideoUpload} />
                  </label>
                </div>
            )}
          </div>
    ...
    case 'Audio':
        return (
            <div className="mt-4">
            <label className="block text-sm font-medium text-gray-400 mb-2">Upload Audio</label>
            {audioPreview ? (
                <div className="relative">
                ...
                <div className="flex items-center justify-center w-full">
                    <label htmlFor="audio-dropzone-file" className="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-800 hover:bg-gray-700 transition-colors">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <MusicIcon />
                        <p className="mt-2 text-sm text-gray-400"><span className="font-semibold">Click to upload</span> or drag and drop</p>
                        <p className="text-xs text-gray-500">MP3, WAV, OGG</p>
                    </div>
                    <input id="audio-dropzone-file" type="file" className="hidden" accept="audio/*" onChange={handleAudioUpload} />
                    </label>
                </div>
            )}
            </div>
      ...
      case 'Quote':
        return (
          <div className="mt-4">
            <label htmlFor="quote-source" className="block text-sm font-medium text-gray-400 mb-2">Quote Source</label>
            <input type="text" id="quote-source" className="w-full bg-gray-900 border border-gray-600 rounded-md p-2 text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Albert Einstein" />
          </div>
        );
      case 'Link':
        return (
          <div className="mt-4">
            <label htmlFor="link-url" className="block text-sm font-medium text-gray-400 mb-2">Link URL</label>
            <input type="url" id="link-url" className="w-full bg-gray-900 border border-gray-600 rounded-md p-2 text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="https://example.com" />
          </div>
        );
      default:
        return null;
  ...
  return (
    <div className="bg-gray-900 min-h-screen lg:h-screen lg:overflow-y-hidden text-white font-sans flex flex-col lg:flex-row p-4 sm:p-6 lg:p-8 gap-6 relative">
      {/* Notification */}
      {notification && (
        <div className={`fixed top-5 right-5 z-50 p-4 rounded-lg shadow-lg text-white ${notification.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
      ...
      {isMarkdownHelpOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div ref={markdownHelpRef} className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">Markdown Cheatsheet</h2>
                <button onClick={() => setIsMarkdownHelpOpen(false)} className="text-gray-400 hover:text-white">
                    <XIcon />
                </button>
            </div>
            <ul className="space-y-3">
                {markdownCheatsheet.map(item => (
                    <li key={item.syntax} className="flex items-center justify-between p-2 bg-gray-900 rounded-md">
                        <code className="text-sm text-cyan-400 bg-gray-700 px-2 py-1 rounded">{item.syntax}</code>
      ...
      <main className="flex-1 lg:flex-[3] bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col lg:overflow-y-auto">
        ...
        <div className="border border-gray-600 rounded-lg flex-1 flex flex-col">
          <div className="flex items-center p-2 bg-gray-900 rounded-t-lg border-b border-gray-600 gap-2 flex-wrap">
            <button onClick={() => setIsMarkdownHelpOpen(true)} className="p-2 rounded hover:bg-gray-700"><MarkdownIcon /></button>
            <div className="w-px h-5 bg-gray-600 mx-1"></div>
            <button onClick={() => handleFormat('h1')} className="p-2 rounded hover:bg-gray-700"><Heading1Icon /></button>
            <button onClick={() => handleFormat('h2')} className="p-2 rounded hover:bg-gray-700"><Heading2Icon /></button>
            <div className="w-px h-5 bg-gray-600 mx-1"></div>
            ...
            <button onClick={() => handleFormat('code')} className="p-2 rounded hover:bg-gray-700"><CodeIcon /></button>
            <button onClick={() => handleFormat('link')} className="p-2 rounded hover:bg-gray-700"><LinkIcon /></button>
            <div className="w-px h-5 bg-gray-600 mx-1"></div>
            <button onClick={() => handleFormat('quote')} className="p-2 rounded hover:bg-gray-700"><QuoteIcon /></button>
            <button onClick={() => handleFormat('ul')} className="p-2 rounded hover:bg-gray-700"><ListIcon /></button>
            <button onClick={() => handleFormat('ol')} className="p-2 rounded hover:bg-gray-700"><ListOrderedIcon /></button>
            <button onClick={() => handleFormat('hr')} className="p-2 rounded hover:bg-gray-700"><MinusIcon /></button>
            <div className="w-px h-5 bg-gray-600 mx-1"></div>
            <button onClick={() => setIsPreview(!isPreview)} className={`p-2 rounded ${isPreview ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}><EyeIcon /></button>
          </div>
          {isPreview ? (
            <div 
      ...
      <aside className="lg:flex-1 bg-gray-800 p-6 rounded-lg shadow-lg h-fit lg:overflow-y-auto">
        ...
        <div className="mb-4">
          <label htmlFor="feather" className="block text-sm font-medium text-gray-400 mb-2">Feather</label>
          <select
            id="feather"
            value={feather}
                ...
                <div className="absolute z-10 w-full mt-1 bg-gray-900 border border-gray-600 rounded-md shadow-lg">
                    ...
                    <ul className="max-h-40 overflow-y-auto p-2">
                        {availableTags.length > 0 ? availableTags.map(tag => (
                            <li 
                                key={tag} 
                                className={`p-2 rounded-md cursor-pointer ${selectedTags.includes(tag) ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}
                                onClick={() => selectedTags.includes(tag) ? handleTagRemove(tag) : handleTagSelect(tag)}
                            >
                                {tag}
                            </li>
                        )) : <li className="p-2 text-gray-500">Select a category first.</li>}
                    </ul>
                </div>
            )}
             <div className="flex flex-wrap gap-2 mt-2">
                {selectedTags.map(tag => (
                    <div key={tag} className="bg-blue-600/50 text-blue-100 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1">
                        {tag}
                        <button onClick={() => handleTagRemove(tag)} className="hover:text-white"><XIcon/></button>
                    </div>
                ))}
            </div>
        </div>
        
        <button onClick={handlePublish} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md transition-colors">
          Publish
        </button>
      </aside>
    </div>
